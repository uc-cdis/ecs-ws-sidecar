...
function populate() {
    ...
    COUNT=0
    curl -H "Authorization: Bearer ${ACCESS_TOKEN}" "https://$GEN3_ENDPOINT/manifests/" 2>/dev/null | jq -c ".manifests | reverse | .[]" | while read i; do
...
function clean_up_old_mounts() {
    ...
    while [ $(ls -ldrt /data/manifest*/ | wc -l) -gt $MAX_MANIFESTS ]; do
        OLDDIR=$(find /data -type d -name "manifest*" | while read line; do echo "$(date +%s -r "$line") $line"; done | sort | head -n 1 | cut -d ' ' -f 2)
